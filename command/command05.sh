...
wget -O ~/exercise02/data/d_melanogaster_old.genes.gtf.gz "ftp://ftp.ensembl.org/pub//release-89/gtf/drosophila_melanogaster/Drosophila_melanogaster.BDGP6.89.gtf.gz"
gunzip ~/exercise02/data/d_melanogaster_old.genes.gtf
# 2. Count the genes in the positive (+) and negative (-) strand of old D. melanogaster genome
#    and save the counts to result05_2.csv.
grep -v "^#" ./data/d_melanogaster_old.genes.gtf | 
awk 'BEGIN {OFS=","} {arr[$7]+=1} END {print arr["+"],arr["-"]}' > ~/exercise02/result/result05_2.csv
