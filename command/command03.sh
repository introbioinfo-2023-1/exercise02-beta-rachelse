...
grep -v "^#" ~/exercise02/data/d_melanogaster.genes.gtf | awk '$1!~/Scaffold/ {print $1}' | sort | uniq > ~/exercise02/result/result03_1.txt
...
grep -v "^#" ~/exercise02/data/d_melanogaster.genes.gtf | awk '$1!~/Scaffold/ {print $1}' | sort | uniq -c | awk 'BEGIN{OFS=","} $1>=700{print $2, $1}' > ~/exercise02/result/result03_2.csv
